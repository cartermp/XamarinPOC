...
namespace POC_Android
{
    ...
    public class MainActivity : Activity, IGoogleApiClientConnectionCallbacks, IGoogleApiClientOnConnectionFailedListener
    {
        ...
        protected override void OnCreate(Bundle bundle)
        {
            ...
            mTextView = FindViewById<TextView>(Resource.Id.TextWebThing);
        ...
        private void DoOnClickhandlers()
        {
            ...
            cameraButton.Click += delegate { if (AppForTakingPicturesExists()) { LauchCamera(); } };
            ...
            webButton.Click += async (sender, e) =>
            {
                using (var client = new HttpClient())
                {
                    var response = await client.GetAsync("http://www.example.com");
                    mTextView.Text = response.StatusCode.ToString();
                }
            };
        }
        ...
        /// <summary>
        /// Queries activities which could take a picture, thereby informing us if a camera app is installed.
        /// </summary>
        private bool AppForTakingPicturesExists()
        {
            Intent intent = new Intent(MediaStore.ActionImageCapture);
            var availableActivities = PackageManager.QueryIntentActivities(intent, PackageInfoFlags.MatchDefaultOnly);

            return availableActivities.HasAny();
        }
        ...
        {
            var geoUri = Android.Net.Uri.Parse("google.navigation:q=119 Horstman Street");
